...
const ModalAcct = props => {
  ...
  const archiveMessage =
    'Are you sure that you want to Archive the dates selected.  this will move these dates to an archive location';
  const diaryMessage =
    'Are you sure that you want to Delete all of your data.  this will remove all of your current data and you will have to start a new Calculator and Dairy';
  const accountMessage =
    'Are you sure that you want to Delete your accout.  You will no longer be able to login and will need to create a new account';
    ...
    if (modalState.myValue === typeText) {
      ...
      switch (modalState.myValue) {
        case 'archive':
          ...
          currentDate = dayjs(`${dateRange[0].currentDate}`).format(
            `MM/DD/YYYY`
          );
          reportDates = {
            startDate: startDate,
        ...
        <Fade in={modalState.open}>
          <Box>
            <div className={s.modalBox}>
              ...
              {modalState.myValue === 'archive' ? (
                <h3> {archiveMessage}</h3>
              ) : modalState.myValue === 'dairy' ? (
                <h3> {diaryMessage}</h3>
              ) : modalState.myValue === 'acct' ? (
                <h3> {accountMessage}</h3>
              ) : (
                <h3> {getArchivedMessage}</h3>
              )}
              <p className={s.ptag}>
                type{' '}
                <span className={s.messageSpan}> {modalState.myValue}</span> and
                click submit.
              </p>
              <TextField
                id="outlined-basic"
                label={modalState.myValue}
                variant="standard"
